    ...
    function getKarmaStartFunction(karma) {
        ...
        return function() {
            ...
            if (window['TP'] && TP.boot) {
                ...
                //  the config block via the config.args value.
                if (karma.config && karma.config.args) {
                    args = JSON.parse(karma.config.args[0]);
                }

                //  Capture values from configuration data to augment baseline.
                //  NOTE that the data here DOES NOT INCLUDE tibet.json DATA!
                //  We haven't launched yet so it hasn't been loaded yet.
                cfg = TP.sys.cfg('karma');

                [cfg, args].forEach(function(obj) {
                    Object.keys(obj).forEach(function(key) {
                        var value,
                            slot;

                        value = obj[key];
                        if (key.indexOf('karma.boot') === 0) {
                ...
                if (params['karma.debug']) {
                    copied = JSON.parse(JSON.stringify(karma.config));
                    copied.args = params;
                    karma.info({
                        log: ['karma config: ' +
